...
// 1. 인가 코드로 카카오 액세스 토큰 발급 (서버 간 통신)
...
export function reissueAccessToken(refreshToken: string): string {
  ...
  return jwt.sign(
    { userId: decoded.userId } satisfies JwtPayload,
    CONFIG.JWT_SECRET,
    { expiresIn: ACCESS_TOKEN_EXPIRES_IN },
  );
}

// 6. [메인 로직] 카카오 로그인/회원가입 통합 처리
